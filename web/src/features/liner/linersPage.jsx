...
const EditToolbar = ({ setRows, setRowModesModel }) => {
  const handleClick = () => {
    setRows((oldRows) => [
      ...oldRows,
      { iata_code: "", model_code: "", isNew: true },
    ]);
    setRowModesModel((oldModel) => ({
      ...oldModel,
      "": { mode: GridRowModes.Edit, fieldToFocus: "iata_code" },
    }));
  };
...
export const LinersPage = () => {
  ...
  const { data, isLoading } = useGetLinersQuery({
    page: page + 1,
    count: rowCount,
  });
  ...
  const {
    data: liner,
    isLoading: isLoadingLiner,
    error,
  } = useGetLinerByCodeQuery({ code: searchQuery });
  ...
  const [updateLiner, { isLoadingUpdate }] = useUpdateLinerByCodeMutation();
  const [deleteLiner, { isLoadingDelete }] = useDeleteLinerByCodeMutation();
  const [createLiner, { isLoadingCreate }] = useCreateLinerMutation();
  ...
  return (
    <>
      ...
      <Grid rowSpacing={3} columnSpacing={3} container>
        ...
        <Grid item xs={5}>
          {isLoadingLiner && <CircularProgress />}
          {!isLoadingLiner && error && (
            <Typography>
              Ошибка! Код: {error?.status}. Сообщение: {error?.data?.error}
            </Typography>
          )}
          {!isLoadingLiner && !error && liner && searchQuery !== "" && (
            <EntityInfo
              items={columns
                .filter((col) => col.type !== "actions")
                .map((col) => {
                  return { label: col.headerName, value: liner[col.field] };
                })}
              onDelete={() =>
                deleteLiner({ code: liner.iata_code })
                  .unwrap()
                  .catch(({ data: { error } }) => alert(error))
              }
            />
